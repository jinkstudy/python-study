"""
    [ dictionary 형 ]

    1- 키와 값으로 구성 ( 자바의 map 와 유사 )
    2- 저장된 자료의 순서는 의미 없음
    3- 중괄호 {} 사용
    4- 변경가능

    ` 사전.keys() : key만 추출 (임의의 순서)
    ` 사전.values() : value만 추출 (임의의 순서)
    ` 사전.items() : key와 value를 튜플로 추출 (임의의 순서)
"""

print('--------- 1. 딕셔너리 요소 --------------- ')
dt = {1:'one', 2:'two', '3':'three'}
print(dt)  #{1: 'one', 2: 'two', '3': 'three'}
print(dt.get(1)) #one
print(dt[1])  #one
print(dt['3']) #three
print(dt.get('3')) #three

dt = {1:'one', 2:'two', '3':'three',1:'again'} # key값 중복입력하는 경우, 뒤에 것으로 바뀐다.
print(dt) #{1: 'again', 2: 'two', '3': 'three'}



# 키는 숫자와 문자 그리고 튜플이여야 한다. 즉 리스트는 안된다.
# 리스트의  값이 변경 가능하다. 그러나 키값을 변경하면 안되므로 리스는 안된다
dt2 = {1:'one', 2:'two', (3,4):'tuple'}
print(dt2.get((3,4)))
print(dt2[(3,4)])

print('--------- 2. 딕셔너리 추가 및 수정  --------------- ')
dt2['korea'] = 'seoul' #key값이 없으면 추가
print(dt2)


dt2['korea'] = 'incheon' #key값이 있으면 수정
print(dt2)
# 여러개 추가할 때
#(update()이용)

dt2.update({'11':'busan','22': 'changwon'})
print(dt2)

print('--------- 3. Key로 Value값 찾기  --------------- ')
# print(dt2[3]) #존재하지 않는 경우  --> error
print(dt2.get(3)) #존재하지 않아도 에러 발생 x --> none
print(dt2.get(3,"없음")) #없는 경우 원하는 문구로 출력가능






# Key와 Value만 따로 검색
print(dt2.values())
print(dt2.keys())
print(dt2.items())
